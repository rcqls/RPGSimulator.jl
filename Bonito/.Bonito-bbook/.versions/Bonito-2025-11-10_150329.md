...
function interactive_pv_simulation()
    ...
    # Figure principale
    fig = WGLMakie.Figure(resolution = (900, 600))
    
    # Graphique
    ax = WGLMakie.Axis(fig[1,1], title="Évolution des PV", xlabel="Round", ylabel="PV",
              limits=((0,3), (0, max(pv_jed[], pv_gob[])+50)))
    
    # Lignes de PV
    line_jed = WGLMakie.lines!(ax, xdata, ydata_jed; color=:blue, linewidth=2)
    line_gob = WGLMakie.lines!(ax, xdata, ydata_gob; color=:red, linewidth=2)
    ...
    # Sliders HTML via Bonito
    sliders = Bonito.DOM.div(
        "Jed PV: ", Bonito.DOM.input(; type="range", min=0, max=500, value=pv_jed[]),
        "Jed ATK: ", Bonito.DOM.input(; type="range", min=0, max=100, value=atk_jed[]),
        "Jed DEF: ", Bonito.DOM.input(; type="range", min=0, max=100, value=def_jed[]),
        "Gob PV: ", Bonito.DOM.input(; type="range", min=0, max=500, value=pv_gob[]),
        "Gob ATK: ", Bonito.DOM.input(; type="range", min=0, max=100, value=atk_gob[]),
        "Gob DEF: ", Bonito.DOM.input(; type="range", min=0, max=100, value=def_gob[])
    )
    ...
    inputs.forEach(inp => inp.addEventListener('input', ()=>{
        $pv_jed.value = parseInt(inputs[0].value);
        $atk_jed.value = parseInt(inputs[1].value);
        $def_jed.value = parseInt(inputs[2].value);
        $pv_gob.value = parseInt(inputs[3].value);
        $atk_gob.value = parseInt(inputs[4].value);
        $def_gob.value = parseInt(inputs[5].value);
        $update_lines!();
    }));
    """
