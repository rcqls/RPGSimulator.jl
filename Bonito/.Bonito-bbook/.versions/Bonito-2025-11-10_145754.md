...
# --- Observables pour les stats ---
archer_atk = Observable(30)
archer_def = Observable(10)
gobelin_atk = Observable(35)
gobelin_def = Observable(5)

# --- HTML sliders ---
archer_atk_slider = DOM.input(; type="range", min=1, max=100, value=archer_atk[])
archer_def_slider = DOM.input(; type="range", min=1, max=50, value=archer_def[])
gobelin_atk_slider = DOM.input(; type="range", min=1, max=100, value=gobelin_atk[])
gobelin_def_slider = DOM.input(; type="range", min=1, max=50, value=gobelin_def[])

# --- Figure WGLMakie ---
fig = WGLMakie.Figure(resolution=(800,400))
...
function update_graph!()
    ...
    for name in unique(defense_summary.actor)
        ...
        if haskey(lines_dict, name)
            ...
            lines_dict[name] = WGLMakie.lines!(ax, sub.round, sub.defense_mean, label=name, linewidth=2)
            WGLMakie.scatter!(ax, sub.round, sub.defense_mean, color=:auto)
        end
    end
