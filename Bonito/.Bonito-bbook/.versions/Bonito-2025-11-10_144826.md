...
for name in unique(defense_summary.actor)
    sub = filter(:actor => ==(name), defense_summary)
    push!(traces, scatter(x=sub.round, y=sub.defense_mean,
                          mode="lines+markers", name=name))
end

plt = Plot(traces, Layout(title="Défense moyenne par round",
                          xaxis_title="Round", yaxis_title="Défense"))

# --- Export HTML complet en string ---
html_str = PlotlyJS.savefig(plt, "defense_plot.html", format=:html, include_plotlyjs=true)
html_str = read("defense_plot.html", String)  # lit le contenu HTML complet

# --- Affichage dans Bonito via raw_html ---
Bonito.DOM.div(
    Bonito.DOM.h2("Résumé des combats"),
    Bonito.Table(summary),
    Bonito.DOM.h2("Défense moyenne par round"),
