...
using Bonito, Tables
using Plots
plotlyjs()  # Backend PlotlyJS pour interactivité
...
# --- Création du graphique avec Plots.jl / PlotlyJS ---
fig = plot(title="Défense moyenne par round", xlabel="Round", ylabel="Défense")
for name in unique(defense_summary.actor)
    sub = filter(:actor => ==(name), defense_summary)
    plot!(sub.round, sub.defense_mean, label=name, marker=:circle, linewidth=2)
end
...
# Supposons que summary contient les PV restants de chaque personnage
# Exemple : summary = DataFrame(combat=[1,2,3,4,5], archer_PV=[70, 60, 80, 50, 90], gobelin_PV=[0, 20, 10, 0, 30])

# Créer le graphique
fig = plot(
    scatter(x=summary.combat, y=summary.archer_PV, mode="lines+markers", name="Archer PV"),
    scatter(x=summary.combat, y=summary.gobelin_PV, mode="lines+markers", name="Gobelin PV"),
    Layout(title="PV restants après chaque combat",
           xaxis_title="Combat",
           yaxis_title="PV")
)
