...
using WGLMakie
using Observables
include("../Examples/test.jl")  # ou chemin vers RPGSimulator

WGLMakie.activate!(; resize_to_body=true)

function interactive_pv_simulation()
    # Création personnages (exemple)
    archer = Archer(nom="Jed")
    archer.stats = Stats(PV=100, PM=20, ATK=30, VITESSE=40, DEFENSE=10)

    gobelin = Gobelin(nom="Hussin")
    gobelin.stats = Stats(PV=120, PM=0, ATK=25, VITESSE=35, DEFENSE=5)

    # Observables pour Jed
    pv_jed  = Observable(archer.stats.PV)
    atk_jed = Observable(archer.stats.ATK)
    def_jed = Observable(archer.stats.DEFENSE)

    # Observables pour Gobelin
    pv_gob  = Observable(gobelin.stats.PV)
    atk_gob = Observable(gobelin.stats.ATK)
    def_gob = Observable(gobelin.stats.DEFENSE)

    # Observables pour lignes graphiques
    xdata = Observable(Float64[])
    ydata_jed = Observable(Float64[])
    ydata_gob = Observable(Float64[])
    ...
    # Légende
    Legend(fig[1,2], [line_jed, line_gob])

    # Fonction calcul trajectoire PV par round (3 rounds)
    function compute_pv_traj(pv, atk, defn; rounds=3)
        traj = Float64[]
        current = pv
        push!(traj, current)
    ...
    # Mise à jour réactive graphique
    function update_lines!()
        rounds, traj_jed = compute_pv_traj(pv_jed[], atk_gob[], def_jed[], rounds=3)
        _, traj_gob = compute_pv_traj(pv_gob[], atk_jed[], def_gob[], rounds=3)
        xdata[] = collect(rounds)
        ydata_jed[] = traj_jed
        ydata_gob[] = traj_gob
        # Ajustement limites Y pour dézoom auto
    ...
    sg_jed = SliderGrid(
        fig[2,1],
        (label="Jed PV", range=0:500, startvalue=archer.stats.PV),
        (label="Jed ATK", range=0:100, startvalue=archer.stats.ATK),
        (label="Jed DEF", range=0:100, startvalue=archer.stats.DEFENSE)
    )
    sPV_jed, sATK_jed, sDEF_jed = sg_jed.sliders
    on(sPV_jed.value) do v pv_jed[] = v; update_lines!() end
    on(sATK_jed.value) do v atk_jed[] = v; update_lines!() end
    ...
    sg_gob = SliderGrid(
        fig[3,1],
        (label="Gob PV", range=0:500, startvalue=gobelin.stats.PV),
        (label="Gob ATK", range=0:100, startvalue=gobelin.stats.ATK),
        (label="Gob DEF", range=0:100, startvalue=gobelin.stats.DEFENSE)
    )
    sPV_gob, sATK_gob, sDEF_gob = sg_gob.sliders
    on(sPV_gob.value) do v pv_gob[] = v; update_lines!() end
    on(sATK_gob.value) do v atk_gob[] = v; update_lines!() end
...
# Afficher la simulation interactive en Bonito
Bonito.DOM.div(
    Bonito.DOM.h2("Simulation de PV interactive"),
    interactive_pv_simulation()
)
