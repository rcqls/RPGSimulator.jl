...
# --- Crée les sliders en HTML + Observables ---
archer_atk = Observables.Observable(30)
archer_def = Observables.Observable(10)
gobelin_atk = Observables.Observable(35)
gobelin_def = Observables.Observable(5)

archer_atk_slider = slider = DOM.input(; type="range", min=1, max=100, value=archer_atk[])
archer_def_slider = slider = DOM.input(; type="range", min=1, max=50, value=archer_def[])
gobelin_atk_slider = slider = DOM.input(; type="range", min=1, max=100, value=gobelin_atk[])
gobelin_def_slider = slider = DOM.input(; type="range", min=1, max=50, value=gobelin_def[])

# --- Figure WGLMakie ---
fig = Figure(resolution=(800,400))
ax = Axis(fig[1,1], title="Défense moyenne par round", xlabel="Round", ylabel="Défense")

lines_dict = Dict{String, Lines}()

# --- Fonction de mise à jour ---
function update_graph()
    # Créer les personnages avec les valeurs actuelles
    archer = Archer(nom="Jed")
    archer.stats = Stats(PV=100, PM=20, ATK=archer_atk[], VITESSE=40, DEFENSE=archer_def[])
    gobelin = Gobelin(nom="Hussin")
    gobelin.stats = Stats(PV=120, PM=0, ATK=gobelin_atk[], VITESSE=40, DEFENSE=gobelin_def[])
    ...
    for name in unique(defense_summary.actor)
        sub = filter(:actor => ==(name), defense_summary)
        if haskey(lines_dict, name)
            lines_dict[name].x = sub.round
            lines_dict[name].y = sub.defense_mean
        else
            lines_dict[name] = lines!(ax, sub.round, sub.defense_mean, label=name, linewidth=2)
...
# --- JS integration : mettre à jour les Observables quand les sliders changent ---
js_script = js"""
[[$archer_atk_slider, $archer_def_slider, $gobelin_atk_slider, $gobelin_def_slider]].forEach(slider=>{
    slider.addEventListener('input', ()=>{
        $archer_atk.value = parseInt($archer_atk_slider.value);
        $archer_def.value = parseInt($archer_def_slider.value);
        $gobelin_atk.value = parseInt($gobelin_atk_slider.value);
        $gobelin_def.value = parseInt($gobelin_def_slider.value);
        $update_graph();
    });
});
"""
...
Bonito.DOM.div(
    Bonito.DOM.h2("Défense moyenne par round"),
    DOM.div(["Archer ATK: ", archer_atk_slider]),
    DOM.div(["Archer DEF: ", archer_def_slider]),
    DOM.div(["Gobelin ATK: ", gobelin_atk_slider]),
    DOM.div(["Gobelin DEF: ", gobelin_def_slider]),
    fig, js_script;
