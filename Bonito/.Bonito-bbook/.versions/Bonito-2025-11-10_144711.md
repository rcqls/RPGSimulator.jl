...
# Sauvegarde en image
Plots.savefig(fig, "defense_plot.png")  # PNG interactif limité, mais visible
...
Bonito.DOM.div(
    ...
    Bonito.DOM.img(src="defense_plot.png", width="800px")
)
...
# Créer la figure PlotlyJS
traces = []
for name in unique(defense_summary.actor)
    sub = filter(:actor => ==(name), defense_summary)
    push!(traces, scatter(x=sub.round, y=sub.defense_mean, mode="lines+markers", name=name))
end
plt = Plot(traces, Layout(title="Défense moyenne par round", xaxis_title="Round", yaxis_title="Défense"))

# Convertir en HTML complet
html_str = PlotlyJS.savefig(plt, "defense_plot.html", format=:html, include_plotlyjs=true)  # génère le fichier
html_str = read("defense_plot.html", String)  # lit le contenu HTML
