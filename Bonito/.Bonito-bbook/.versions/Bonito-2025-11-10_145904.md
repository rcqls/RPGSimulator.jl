...
colors = Dict("Jed" => :blue, "Hussin" => :red)  # couleur par personnage
...
function update_graph!()
    ...
    for name in unique(defense_summary.actor)
        ...
        if haskey(lines_dict, name)
            ...
            lines_dict[name] = WGLMakie.lines!(ax, sub.round, sub.defense_mean, label=name, linewidth=2, color=colors[name])
            WGLMakie.scatter!(ax, sub.round, sub.defense_mean, color=colors[name])
        end
    end
